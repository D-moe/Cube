...
function init() {
  ...
  const light1 = new THREE.HemisphereLight(0xffffff, 0x888888, 3);
  const light2 = new THREE.HemisphereLight(0xffffff, 0x888888, 3);
  ...
  // Our green is there blue
  // TODO(morleyd): Old normals
...
async function rotateStr(str) {
  if (str == 'R') {
    ...
    await rotate(new THREE.Vector3(0, 0, 1), false);
    // Our white is their green
    // Our green is their blue
  } else if (str == 'O') {
    await rotate(new THREE.Vector3(1, 0, 0), true);
  } else if (str == 'O*') {
    await rotate(new THREE.Vector3(1, 0, 0), false);
